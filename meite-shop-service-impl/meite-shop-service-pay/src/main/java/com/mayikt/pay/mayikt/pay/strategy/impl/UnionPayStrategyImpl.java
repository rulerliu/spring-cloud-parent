package com.mayikt.pay.mayikt.pay.strategy.impl;
...
import com.mayikt.pay.unionpay.acp.sdk.AcpService;
import com.mayikt.pay.unionpay.acp.sdk.DemoBase;
import com.mayikt.pay.unionpay.acp.sdk.LogUtil;
...
//        resp.setContentType("text/html; charset="+ DemoBase.encoding);
        ...
        requestData.put("orderId", payMentTransacOutDTO.getPaymentId());             //商户订单号，8-40位数字字母，不能含“-”或“_”，可以自行定制规则
        requestData.put("txnTime", format(payMentTransacOutDTO.getCreatedTime()));        //订单发送时间，取系统时间，格式为YYYYMMDDhhmmss，必须取当前时间，否则会报txnTime无效
        requestData.put("currencyCode", "156");         			  //交易币种（境内商户一般是156 人民币）
        requestData.put("txnAmt", payMentTransacOutDTO.getPayAmount() + "");             			      //交易金额，单位分，不要带小数点
        ...
        requestData.put("frontUrl", paymentChannelEntity.getSyncUrl());
//      requestData.put("frontUrl", DemoBase.frontUrl);
        ...
        requestData.put("backUrl", paymentChannelEntity.getAsynUrl());
...
//        resp.getWriter().write(html);
    ...
    private String format(Date createdTime) {
        return new SimpleDateFormat("yyyyMMddHHmmss").format(createdTime);
    }
}
