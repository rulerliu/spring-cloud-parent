package com.mayikt.pay.mq.consumer;
...
public class PayCheckStateConsumer {
	...
	@RabbitListener(queues = PayConstants.PAY_COMPENSATION_QUEUE)
	public void process(Message message, @Headers Map<String, Object> headers, Channel channel) throws IOException {
		try {
			...
			if (PayConstants.PAY_STATUS_SUCCESS.equals(paymentStatus)) {
				log.error(">>>>支付id paymentId:{}, 已经支付成功过", paymentId);
				basicNack(message, channel);
				return;
			}
			...
			int updatePaymentStatus = paymentTransactionMapper.updatePaymentStatus(PayConstants.PAY_STATUS_SUCCESS + "",
